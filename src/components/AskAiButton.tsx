  ...
  return (
    <Dialog open={open} onOpenChange={handleOnOpenChange}>
      <form>
        ...
        >
          <DialogHeader>
            <DialogTitle>Ask Gemini About Your Notes</DialogTitle>
            <DialogDescription>
              Gemini can answer questions about your notes, summarize them, and
          ...
          <div className="mt-4 flex flex-col gap-8">
            {questions.map((question, index) => (
              <Fragment key={index}>
                ...
                {responses[index] && (
                  <p
                    className="bot-response text-xs text-muted-foreground"
                    dangerouslySetInnerHTML={{ __html: responses[index] }}
                  />
                )}
              </Fragment>
            ))}
            {isPending && <p className="text-xs animate-pulse">Thinking...</p>}
          </div>
